...
For convenience only, we will create the network we used in our main [tutorial](https://github.com/chainHero/heroes-service).
...
In order for us to correctly create our network we need few binaries provided by Hyperledger Fabric.
...
Ok, we are done with the crypto-config file. You must be able to see two folders inside the crypto-config folder. It must be ordererOrganizations & peerOrganizations.

### b. Artifacts

In order for us to finish the initialization of our blockchain we need to create : 

- The genesis block. It is the first block of a blockchain. It will define the blockchain.
- A channel. The channel is a kind of private network inside the main network which our users will be able to use.
...
So let's create this configuration file and start editing it :
```
touch configtx.yaml && \
vi configtx.yaml
```
...
Now that our configuration file is ready to use let's create the artifacts folder, which will contain the output files generated by the binaries:
```
mkdir -p artifacts
```
...
> Note that we have to set the environment variable 'FABRIC_CFG_PATH' with the PWD of our current directory because the binary find the
configuration file through an environment variable and not from its directory.

Now that it have been created let's see how we create the channel :
```
./bin/configtxgen -profile ChainHero -outputCreateChannelTx ./artifacts/chainhero.channel.tx -channelID chainhero
```

Finally, we need a peer node which is called **the anchor peer** :
```
./bin/configtxgen -profile ChainHero -outputAnchorPeersUpdate ./artifacts/org1.chainhero.anchors.tx -channelID chainhero -asOrg ChainHeroOrganization1
```
...
In the 'docker-compose.yaml' file you need to replace **BOTH** \*\*HERE** by the name of the file located at : 

`heroes-service-network/crypto-config/peerOrganizations/org1.hf.chainhero.io/ca/`

However, as you might have seen there is 2 files located here, the name of the file you need to copy should be something like that :
